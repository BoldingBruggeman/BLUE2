...
TPXO9_data_dirs = ( '/data', '../../../igotm/data', '/ACQUA/COMMONDATA' )
...
#KB ERA_path = os.path.join(ERA_data_dir, 'ERA-interim-org/analysis_2016.nc')
...
#KBsim.airsea.t2m.set(pygetm.input.from_nc(ERA_path, 't2m', **era_kwargs) - 273.15)
sim.airsea.t2m.set(pygetm.input.from_nc(ERA_path, 't2', **era_kwargs) - 273.15)
#KBsim.airsea.d2m.set(pygetm.input.from_nc(ERA_path, 'd2m', **era_kwargs) - 273.15)
sim.airsea.d2m.set(pygetm.input.from_nc(ERA_path, 'dev2', **era_kwargs) - 273.15)
#KBsim.airsea.sp.set(pygetm.input.from_nc(ERA_path, 'sp', **era_kwargs))
sim.airsea.sp.set(pygetm.input.from_nc(ERA_path, 'slp', **era_kwargs))
...
sim.start(datetime.datetime(2016, 1, 1), timestep=10., split_factor=20)
