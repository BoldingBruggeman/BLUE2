...
if args.initial and sim.runtype == pygetm.BAROCLINIC:
    ...
    sim.temp.set(
        pygetm.input.from_nc(os.path.join(args.input_dir, "bdy_3d.nc"), "temp").isel(
            time=0
        ),
        on_grid=True,
    )
    sim.salt.set(
        pygetm.input.from_nc(os.path.join(args.input_dir, "bdy_3d.nc"), "salt").isel(
            time=0
        ),
        on_grid=True,
    )
...
if args.output and not args.dryrun:
    ...
    output = sim.output_manager.add_netcdf_file(
        os.path.join(args.output_dir, "baltic_2d.nc"),
        interval=datetime.timedelta(hours=1),
        sync_interval=None,
    )
    ...
    if sim.runtype > pygetm.BAROTROPIC_2D:
        output = sim.output_manager.add_netcdf_file(
            os.path.join(args.output_dir, "baltic_3d.nc"),
